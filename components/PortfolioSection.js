...
function Thumb({ item, onClick }) {
  return (
    ...
    >
      <YouTubeThumbnail id={item.id} alt={item.title || 'Portfolio video thumbnail'} className="h-full w-full object-cover opacity-80 transition duration-700 group-hover:scale-105 group-hover:opacity-100" />
      ...
      <div className="relative z-10 mx-auto max-w-7xl">
        ...
        >
          <div className="aspect-video">
            <YouTubeThumbnail id={section.featured.id} alt={section.featured.title || 'Featured video thumbnail'} priority className="h-full w-full object-cover opacity-80 transition duration-1000 group-hover:scale-[1.035] group-hover:opacity-100" />
          </div>
          <div className="absolute inset-0 bg-gradient-to-t from-black via-black/10 to-transparent" />
